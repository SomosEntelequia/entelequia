...
# ============================================
#   HELPER: EQUIVALENCIAS UDM SAP
# ============================================
# Campos hardcodeados del modelo uom.uom (evita consultas a ir.model.fields)
_UOM_MODEL          = 'uom.uom'
_F_GRUPO            = 'grupo_de_unidad_de_medida_sap'   # campo grupo en uom.uom
_F_UNIDAD           = 'name'                             # campo nombre de unidad en uom.uom
_F_EQUIV            = 'cantidad_de_equivalencia'         # campo equivalencia en uom.uom
_FIELD_EQ_PIEZAS    = 'x_studio_equivalencia_en_piezas'
_FIELD_EQ_PAQUETES  = 'x_studio_equivalencia_en_paquetes'
_GROUP_FIELD_CANDIDATES = [
    'grupo_de_unidad_de_medida_sap',
    'x_studio_grupo_de_unidad_de_medida_sap',
]

def _apply_equivalencias_udm_sap(env, product_record):
    """
    Dado un product.template, busca en uom.uom las equivalencias de
    Pieza y Paquete para el grupo SAP del producto y las escribe en
    x_studio_equivalencia_en_piezas / x_studio_equivalencia_en_paquetes.
    """
    # 1) Obtener el grupo del producto
    grupo_val = False
    for gf in _GROUP_FIELD_CANDIDATES:
        if gf in product_record._fields:
            grupo_val = product_record[gf]
            break

    if not grupo_val:
        # Sin grupo -> limpiar equivalencias
        vals_clear = {}
        if _FIELD_EQ_PIEZAS   in product_record._fields: vals_clear[_FIELD_EQ_PIEZAS]   = 0.0
        if _FIELD_EQ_PAQUETES in product_record._fields: vals_clear[_FIELD_EQ_PAQUETES] = 0.0
        if vals_clear:
            product_record.with_context(skip_equivalencias_udm_sap=True).write(vals_clear)
        return

    # 2) Buscar equivalencias Pieza y Paquete en uom.uom usando sudo()
    Pack      = env[_UOM_MODEL].sudo()
    rec_pieza = Pack.search([(_F_GRUPO, "=", grupo_val), (_F_UNIDAD, "ilike", "Pieza")],   limit=1)
    rec_paq   = Pack.search([(_F_GRUPO, "=", grupo_val), (_F_UNIDAD, "ilike", "Paquete")], limit=1)

    eq_pieza = float(rec_pieza[_F_EQUIV] or 0.0) if rec_pieza else 0.0
    eq_paq   = float(rec_paq[_F_EQUIV]   or 0.0) if rec_paq   else 0.0

    vals = {}
    if _FIELD_EQ_PIEZAS   in product_record._fields and float(product_record[_FIELD_EQ_PIEZAS]   or 0.0) != eq_pieza:
        vals[_FIELD_EQ_PIEZAS]   = eq_pieza
    if _FIELD_EQ_PAQUETES in product_record._fields and float(product_record[_FIELD_EQ_PAQUETES] or 0.0) != eq_paq:
        vals[_FIELD_EQ_PAQUETES] = eq_paq

    if vals:
        product_record.with_context(skip_equivalencias_udm_sap=True).write(vals)
        _logger.info("EQUIVALENCIAS UDM aplicadas (Pieza=%s, Paquete=%s) al producto ID %s",
                     eq_pieza, eq_paq, product_record.id)
    else:
        _logger.info("EQUIVALENCIAS UDM sin cambios para producto ID %s", product_record.id)
...
class ApiController(http.Controller):
    ...
    def create_product(self, **kwargs):
        ...
        vals = {
            ...
            "grupo_de_unidad_de_medida_sap": sap_uom_group,
        }
        ...
        try:
            ...
            if existing_product:
                ...
                # ► Disparar equivalencias UDM SAP después de actualizar
                _apply_equivalencias_udm_sap(request.env, existing_product)
                ...
                # ► Disparar equivalencias UDM SAP después de crear
                _apply_equivalencias_udm_sap(request.env, new_product)
    ...
    def update_product(self, **kwargs):
        ...
        try:
            ...
            # ► Disparar equivalencias UDM SAP después de actualizar
            _apply_equivalencias_udm_sap(request.env, template)
